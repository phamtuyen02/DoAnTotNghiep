...
class CartController extends Controller
{
    ...
    public function payment(){
        ...
        $url_provide = 'https://online-gateway.ghn.vn/shiip/public-api/master-data/province'; // Thay thế URL này bằng URL của API
        $token_provide = 'eecd35b7-08f8-11ef-bfe9-c2d25c6518ab'; // Thay thế TOKEN này bằng mã thông báo của bạn

        $response_provide = Http::withToken($token_provide)
            ->get($url_provide);
        if ($response_provide->successful()) {
            $list_provide = $response_provide->json();
        } else {
            $list_provide = [];
            // Xử lý lỗi (nếu có)
        }
        ...
        return view("shop.cart.payment")->with(compact('list_category','list_brand','list_pd_cart','list_provide'));
    }
...
public function shippingUnit(){
    ...
    $list_category = Category::get();
    $list_brand = Brand::get();
    $list_bill = Bill::where('bill.idCustomer', Session::get('idCustomer'))
                     ->orderBy('idBill', 'desc')
                     ->get();

    // Gọi phương thức public để lấy thông tin vận chuyển cho mỗi đơn đặt hàng
    foreach ($list_bill as $bill) {
        $bill->getShippingUnit(); // Gọi phương thức public
        // Kiểm tra thông tin vận chuyển đã được gán vào đơn hàng chưa
        if ($bill->shippingUnit) {
            // Nếu có thông tin vận chuyển, in ra thông tin
            echo "Thông tin vận chuyển cho đơn hàng số " . $bill->idBill . ":";
            echo "<pre>";
            print_r($bill->shippingUnit);
            echo "</pre>";
        } else {
            // Nếu không có thông tin vận chuyển, in ra thông báo
            echo "Đơn hàng số " . $bill->idBill . " chưa có thông tin vận chuyển.";
        }
    }
